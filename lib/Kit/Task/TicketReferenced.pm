      ...
      $args{attachments} = [
        {
          ...
          title_link => $job->app->autotask->executecommand->open_ticket_detail(TicketNumber => $ticket->{TicketNumber}, AccountID => $ticket->{AccountID}),
      ...
      $args{attachments} = [
        {
          ...
          title_link => $job->app->autotask->executecommand->open_task_detail(TaskID => $task->{id}, AccountID => $task->{AccountID}),
        ...
        },
        {
          ...
          title_link => $job->app->autotask->executecommand->open_project(ProjectID => $task->{ProjectID}, AccountID => $task->{AccountID}),
